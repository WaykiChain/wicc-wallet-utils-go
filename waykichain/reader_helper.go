...
func DecodeRawTx(rawTx string,netParams chaincfg.Params) (interface{},error){
	...
	case UCOIN_TRANSFER_TX:
		result,err := DecodeUCoinTransferTx(buf,&netParams)
		if err != nil{
			return nil,err
		}
		return result, nil
...
func DecodeUCoinTransferTx(buf *bytes.Buffer,netParams *chaincfg.Params) (DecodeUCoinTransferParams,error){
	//version
	...
	fee := ReadVarInt(buf)
	//dests
	dests,err := ReadUCoinDestAddr(buf,netParams)
	if err != nil{
...
func ReadUCoinDestAddr(buf *bytes.Buffer,netParams *chaincfg.Params) ([]DecodeUCoinTransferDest,error){
	//数组数
	size := ReadVarInt(buf)
